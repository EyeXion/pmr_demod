...
class HackRF(object):
    ...
    def set_bandwidth(self, bandwidth):
        ...
        basebandFilterBandWidth = (
            libhackrf.hackrf_compute_baseband_filter_bw_round_down_lt(bandwidth)
        )
        result = libhackrf.hackrf_set_baseband_filter_bandwidth(
            self.dev_p, basebandFilterBandWidth
        )
        if result != 0:
            raise IOError(
                "Error code %d when setting bandwidth to %d Hz" % (result, bandwidth)
            )
